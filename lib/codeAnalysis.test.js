...
import { CODE_DATASETS, getDataset } from './codeAnalysis/datasets.js';
...
test('code cards follow the decision domains and stay off choice, score, and boolean', () => {
  ...
  assert.equal(CODE_COACH[0].includes('saved table'), true);
  assert.deepEqual(CODE_DATASETS.map((dataset) => dataset.constant), [
    'supportWeeks',
    'cancelCohorts',
    'rmaLog',
    'weatherHistory',
