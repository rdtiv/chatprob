...
test('pruneForStorage excludes errored turns from the keep budget but only prunes old ones', () => {
  // 21 successful turns (one over the 20-turn budget) with 2 errored turns
  // interleaved. The errored turns must not consume a keep slot, and only the
  // errored turn OLDER than the keep boundary is pruned — a recent aborted
  // partial must not reload wearing the "old enough" storage-limit copy.
  const messages = [];
  for (let i = 0; i < 21; i++) {
    messages.push(makeUserMessage(i));
    messages.push(makeAssistantMessage(i));
  ...
  // error-0 sits before the oldest kept successful turn → pruned;
  // error-12 sits inside the kept window → keeps its alternatives.
  const oldErrored = result.find((message) => message.id === 'error-0');
  const recentErrored = result.find((message) => message.id === 'error-12');
  assert.equal(oldErrored.completions[0].alternativesPruned, true);
  assert.equal(recentErrored.completions[0].alternativesPruned, undefined);
  assert.ok(recentErrored.completions[0].tokenProbabilities[0].top_logprobs);
});
