...
import { getScenario } from './decisionScenarios.js';
...
test('support situations keep the triage lines', () => {
  const decision = decideForScenario(getScenario('cancel-my-plan'), clearAnswers, THRESHOLD_DEFAULTS);
  assert.deepEqual(decision, decideTriage(clearAnswers, THRESHOLD_DEFAULTS));
});

test('a weather move under the reject line rejects and ignores the live-look line', () => {
  const weather = getScenario('weather-tool-gate');
  const decision = decideShaped(
    {
      needsLive: { type: 'boolean', probability: 0.99 },
      move: {
        type: 'choice',
        choice: 'tool',
        probabilities: { memory: 0.4, tool: 0.3, refuse: 0.3 },
      },
    },
    THRESHOLD_DEFAULTS,
    weather.playground,
...
test('one in-between return boolean escalates the plant mail', () => {
  const mail = getScenario('parts-came-back');
  const decision = decideForScenario(
    mail,
    {
      queue: {
        type: 'choice',
        choice: 'quality',
        probabilities: { quality: 0.9, 'sales-credit': 0.05, 'ops-shipping': 0.05, unclear: 0 },
      },
      severity: { type: 'score', score: 1, probabilities: { 0: 0, 1: 1, 2: 0, 3: 0 } },
      repeat: { type: 'boolean', probability: 0.55 },
      creditNotRemake: { type: 'boolean', probability: 0.05 },
    },
    THRESHOLD_DEFAULTS,
  );
  assert.equal(decision.action, 'escalate');
  assert.match(decision.reasons.join(' '), /P\(repeat\)/);
});

test('a clear tool move with a clear live look autos when there is no severity question', () => {
  const weather = getScenario('weather-tool-gate');
  const decision = decideForScenario(
    weather,
    {
      needsLive: { type: 'boolean', probability: 0.95 },
      move: {
        type: 'choice',
        choice: 'tool',
        probabilities: { memory: 0.04, tool: 0.92, refuse: 0.04 },
      },
    },
    THRESHOLD_DEFAULTS,
  );
  assert.equal(decision.action, 'auto');
  assert.equal(decision.reasons.some((line) => /severity/i.test(line)), false);
});
...
test('speed copy uses the measured elapsed and skips a missing clock', () => {
  ...
  assert.equal(
    speedVersusLlm(304),
    '304 ms — a chat reply usually takes seconds because it writes tokens.',
  );
  assert.equal(
    speedVersusLlm(1200),
    '1.2 s — a chat reply usually takes seconds because it writes tokens.',
  );
});
