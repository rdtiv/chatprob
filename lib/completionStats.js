...
/** Path probability under the model — never a claim about truth. */
export function formatJointPathCopy(jointLog10) {
  const odds = formatJointOdds(jointLog10);
  if (!odds) return null;
  return `this path ${odds} under the model — not how true`;
}
