...
export const KEEP_TURNS_DEFAULT = 0; // slider position when the switch is flipped on; memory itself is on by default (KEEP_ALL)

// Note: lib/persistence.js's pruneForStorage also windows "the last N turns",
// but with a different unit (assistant messages with completions, cut on an
// assistant boundary) for a different job (storage quota). The two are
// intentionally separate; neither is a substitute for the other.
