...
test('clampTemperature coerces a numeric string', () => {
  assert.equal(clampTemperature('0.7'), 0.7);
});
...
test('clampSeed returns null outside the 32-bit range', () => {
  ...
  assert.equal(clampSeed(-(2 ** 31) - 1), null);
});

test('clampSeed accepts the exact int32 floor', () => {
  assert.equal(clampSeed(-(2 ** 31)), -(2 ** 31));
});

test('clampSeed coerces a numeric string integer', () => {
