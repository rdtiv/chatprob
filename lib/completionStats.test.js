...
test('formatJointPathCopy wraps the odds as path probability, not truth', () => {
  assert.equal(formatJointPathCopy(-5.7), 'this path ~1 in 10^6 under the model — not how true');
  assert.equal(formatJointPathCopy(-0.1), 'this path better than 1 in 10 under the model — not how true');
  assert.equal(formatJointPathCopy(NaN), null);
  assert.equal(formatJointPathCopy(-4355), null);
});
