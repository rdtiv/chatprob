...
- **Green means expected, not true.** A flat `Likely ≠ true.` stands in the legend beside the swatches, and the **Ask it for a judgment call** chip on the empty screen asks the model to rank the income statement, balance sheet, and cash flow statement by importance — a question with no single right answer. It ranks them anyway, in bright green, without hedging. **Now ask it the other way round** then sends the same three items with the sentence reshaped, and the ranking moves: listed first, the cash flow statement is ranked first in 9 of 9 samples; asked at the end, the income statement is first in 7 of 9. Two confident, incompatible answers, both green — which is the whole claim the legend makes.
