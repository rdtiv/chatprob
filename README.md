...
Deltas are batched into one React update per animation frame, and tokens stay inert until the reply settles — no hovering a card whose alternatives are still arriving. A stream that ends without `done` is finalized as an aborted turn: the partial text stays visible, marked as not part of the conversation, and it is never resent. The send button *is* the Stop button while a reply is in flight — it aborts the same `AbortController` — so stopping a stream lands in exactly that path, and the note on it reads *You stopped this reply.* rather than the dropped-connection default (a mid-stream `error` event still wins, because a real failure is the more useful thing to report). On the whole-reply path (tools on, nothing streaming) there is no partial text to merge into, so the `AbortError` catch appends a whole aborted turn instead — `abortedTurn()` in `lib/abortedTurn.js`, which builds the same shape from scratch with one empty completion, so the note reads *You stopped this reply. No reply arrived.* and the user message is not left hanging without a reply. Clear-chat aborts that same controller and has to stay silent there — the transcript was emptied on purpose — so a `stopRequestedRef` flag set by the Stop button is what tells the two aborts apart, and `abortedFields()` keeps both paths agreeing on what *aborted* means to `Message.js` and to storage pruning.
...
| `lib/abortedTurn.js` | The one shape of an aborted assistant turn, shared by the streaming and JSON paths |
