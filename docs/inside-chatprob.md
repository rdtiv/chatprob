...
This view is the **[live odds](glossary.md#live-odds)**. Move the temperature slider in Controls (on a phone-width sheet, on the card), and watch the percentages move while the rows stay put. Slide toward `0` and the top row climbs to `100.0%` while every other row reads `<0.001%` — that is winner-take-all, drawn. Slide toward `1.8` and the rows flatten toward each other. A line at the bottom of the card keeps the record straight while you do this: *Sampled at 1.0 · showing what-if at 0.3.* The word on screen was drawn at the first number; you are looking at what the odds would have been at the second.
...
The same frozen rows are where **[top-p](glossary.md#top-p-nucleus-sampling)** finally has something to move. Temperature reshapes the odds; top-p cuts the tail. Leave the card pinned and move the sliders in Controls — on a wide window the card stays the original 300px popover, with no sliders of its own. On a phone-width touch screen the card is a sheet and the two sliders sit on it. Move top-p below `1` and watch: the smallest prefix of these rows whose odds add up past `p` stays lit and marked **in**; the rest dim and read **tail**. The rows do not vanish. The percentages you already had — raw under **Of all words**, re-softmaxed under **What-if: only these** — stay the percentages; top-p only says which of them the draw would still be allowed to pick.
...
At `1` nothing is cut and the labels stay off, which is why the card looks the way it did before you touched the slider. Top-p still does not earn a Controls chip — it is a secondary slider, like presence penalty — so the Controls panel is where you read and move it on desktop.
...
3. Leave the card pinned. Open **Controls** and drag **Temp** down toward `0` and watch the rows: the top row goes to `100.0%`, the others to `<0.001%`. Drag it up toward `1.8` and watch them flatten. Read the *Sampled at … · showing what-if at …* line.
4. Still in Controls, drag **Top-p** down from `1` and watch the tail dim: **in** stays, **tail** falls out. Drag temperature while top-p is low — the odds reshape, and which rows stay in the nucleus can change with them. Read the nucleus note.
